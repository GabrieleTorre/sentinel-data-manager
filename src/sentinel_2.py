...
class Indices:
    ...
    def get_valid_data(self, x):
        imgs = np.load(x['data'])
        mask = (np.load(x['masks']) < self.prob_thresh).any(axis=0)
...
class data_manager(Indices):
    ...
    def __init__(self, source_dir, prob_thresh=40, img_thresh=0.6):
    ...
    def validate_cloud_level(self):
        _funct_ = lambda x: self.__validate_weather__(np.load(x)[0])
        return self.df_records.masks.apply(_funct_)

    def validate_snow_level(self):
        _funct_ = lambda x: self.__validate_weather__(np.load(x)[1])
        return self.df_records.masks.apply(_funct_)

    def get_data(self):
        return np.stack([np.load(x) for x in self.data_fnames], axis=0)

    def get_index_ts(self, function, veg_type=None):
        index, values = [], []
        veg_mask = np.load(self.segm_fname)

        for i, row in self.df_records.iterrows():

            VI, MK = function(row)
            if veg_type:
                VMK = (veg_mask == veg_type)
            else:
                VMK = (veg_mask != 0) & (veg_mask != 19)
